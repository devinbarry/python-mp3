...
good_frame_data = bytearray( \
    '\xff\xfb\x90\x64\x76\x00\x05\x34\x66\x50\xf6\x69\x20\x00' \
...
# This isn't really true. Should be file size - 8
riff_frame = 'RIFF%sWAVE' % struct.pack('<L', len(riff_chunk))
riff_frame += riff_chunk
...
class GoodDataTestCase(unittest.TestCase):
    ...
    def test_framedata(self):
        # old-style
        self.assertEquals(good_frame_data[4:],
                          mp3.framedata(good_frame_data, 0, good_frame_header))
